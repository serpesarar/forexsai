...
const SYMBOL = "GDAXI.INDX";
const SYMBOL_LABEL = "DAX";
const PATTERN_SYMBOL = "DAX" as const;
const STORAGE_KEY = "dax-dashboard-layout-v1";
...
function MaximizeWrapper({ children, title, tooltipText, isMaximized, onMaximize }: MaximizeWrapperProps) {
  ...
  return (
    <div className="relative">
      <button
        onClick={onMaximize}
        className="absolute -top-3 right-6 z-20 flex h-7 w-7 items-center justify-center rounded-full border border-slate-700 bg-slate-800 shadow-lg hover:border-blue-500/50"
      >
...
function DaxPageContent() {
  ...
  const daxTicker = useMemo(() => tickers.find((item) => item.label === SYMBOL_LABEL), [tickers]);
  const isPositive = (daxTicker?.change || "").startsWith("+");

  const copy = {
    title: locale === "tr" ? "DAX Komuta Merkezi" : "DAX Command Center",
    subtitle: locale === "tr" ? "GDAXI.INDX için tüm ana analiz panelleri." : "All major GDAXI.INDX analysis panels.",
      ...
      <DashboardEditProvider storageKey={STORAGE_KEY} defaultLayout={GRID_LAYOUT}>
        <div className="relative z-10 mx-auto flex w-full flex-col gap-4 px-3 sm:px-4 md:px-6 lg:px-8 py-4 md:py-6">
          ...
          <motion.div {...frame(0)} className="flex items-center justify-between rounded-3xl border border-slate-800 bg-slate-950/85 p-5 shadow-2xl backdrop-blur-xl md:p-6">
            ...
            <div className="flex items-center gap-3">
              <span className="text-2xl font-black text-white">
                {isLoading || !daxTicker ? "--" : `$${daxTicker.price}`}
              </span>
              {!isLoading && daxTicker?.change && (
          ...
          <motion.div {...frame(3)}>
            ...
            <DaxEditableGrid locale={locale} maximizedCard={maximizedCard} onMaximize={setMaximizedCard} />
          </motion.div>
          ...
          <motion.section {...frame(5)} className="rounded-3xl border border-slate-800 bg-slate-950/85 p-5 shadow-2xl backdrop-blur-xl md:p-6">
            <div className="mb-5 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400">{locale === "tr" ? "DAX tahmin geçmişi" : "DAX prediction history"}</p>
              </div>
            </div>
            <LazyPanel fallbackHeight={520}>
