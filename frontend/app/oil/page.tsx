...
const SYMBOL = "USOIL.FOREX";
const SYMBOL_LABEL = "US OIL";
const PATTERN_SYMBOL = "USOIL" as const;
const STORAGE_KEY = "oil-dashboard-layout-v1";
...
function MaximizeWrapper({ children, title, tooltipText, isMaximized, onMaximize }: MaximizeWrapperProps) {
  ...
  return (
    <div className="relative">
      <button
        onClick={onMaximize}
        className="absolute -top-3 right-6 z-20 flex h-7 w-7 items-center justify-center rounded-full border border-slate-700 bg-slate-800 shadow-lg hover:border-orange-500/50"
      >
...
function OilPageContent() {
  ...
  const oilTicker = useMemo(() => tickers.find((item) => item.label === SYMBOL_LABEL), [tickers]);
  const isPositive = (oilTicker?.change || "").startsWith("+");

  const copy = {
    title: locale === "tr" ? "Petrol (WTI) Komuta Merkezi" : "Oil (WTI) Command Center",
    subtitle: locale === "tr" ? "USOIL.FOREX için tüm ana analiz panelleri." : "All major USOIL.FOREX analysis panels.",
          ...
          <motion.div {...frame(0)} className="flex items-center justify-between rounded-3xl border border-slate-800 bg-slate-950/85 p-5 shadow-2xl backdrop-blur-xl md:p-6">
            ...
            <div className="flex items-center gap-3">
              <span className="text-2xl font-black text-white">
                {isLoading || !oilTicker ? "--" : `$${oilTicker.price}`}
              </span>
              {!isLoading && oilTicker?.change && (
          ...
          <motion.div {...frame(3)}>
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                <Grip className="h-5 w-5 text-orange-400" />
                Panel Grid
              </h2>
            ...
            <OilEditableGrid locale={locale} maximizedCard={maximizedCard} onMaximize={setMaximizedCard} />
          </motion.div>
          ...
          <motion.section {...frame(5)} className="rounded-3xl border border-slate-800 bg-slate-950/85 p-5 shadow-2xl backdrop-blur-xl md:p-6">
            <div className="mb-5 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400">{locale === "tr" ? "Petrol tahmin geçmişi" : "Oil prediction history"}</p>
              </div>
            </div>
            <LazyPanel fallbackHeight={520}>
