...
export default function XauusdPanel() {
    ...
    <div className="glass-premium p-6 space-y-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <h3 className="text-lg font-semibold">Gold Momentum Radar</h3>
        </div>
        <button
          onClick={() => refetch()}
          className="p-2 rounded-full hover:bg-white/10 transition"
          aria-label="Refresh XAUUSD"
        >
      ...
      {isLoading ? (
        ...
        <div className="text-sm text-danger">XAUUSD verisi alınamadı. Lütfen tekrar deneyin.</div>
      ) : (
        <>
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 h-2 bg-white/10 rounded-full">
              <div className="h-2 rounded-full bg-warning" style={{ width: `${data.confidence * 100}%` }} />
            </div>
