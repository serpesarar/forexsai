<!--
Kimi Code context file — verbatim port of /CLAUDE-REASONING.md ("ForexSAI — Reasoning &
Thinking Protocols"). In the original, /CLAUDE.md imports this file via `@CLAUDE-REASONING.md`.
For Kimi Code, .kimi/AGENTS.md points here directly. Content unmodified except this header and
the title line. Source file was NOT edited to produce this copy.
-->

# ForexSAI — Reasoning & Thinking Protocols (Kimi Code)

> Ported from `/CLAUDE-REASONING.md`. Concrete thinking protocols so extended-thinking capacity
> (Claude's Opus extended thinking, or Kimi's equivalent deep-reasoning mode) is spent where it
> actually matters on this project, not spread evenly across every task.
...
1. **Okudun mu?** — dosyanın güncel halini gör, hafızandan çalışma
...
Cevaplarda belirsizlik varsa: önce ilgili dosyaları tara, sonra düzenle.
...
**Devam et (sorma):**
- Cevabı proje bağlamından (`.kimi/AGENTS.md`, `.kimi/context/master-config.md`) veya proje
  mantığından çıkarabiliyorsan
...
- Supabase schema'nın güncel hali (önce sorgulayarak bak)
...
| Cache TTL değişikliği | master-config.md'deki Cache TTL tablosunu güncelle |
