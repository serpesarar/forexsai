<!--
Kimi Code context file — verbatim port of /CLAUDE.md ("ForexSAI — Claude Code Master
Configuration"). Content is unmodified from the source file except for this header block and
the title line below. The original /CLAUDE.md remains the source of truth for Claude Code
sessions and was NOT edited to produce this copy. Re-sync this file whenever /CLAUDE.md changes.
Reasoning protocols live in the sibling file reasoning-protocols.md (ported from
/CLAUDE-REASONING.md, which /CLAUDE.md imports via @CLAUDE-REASONING.md).
-->

# ForexSAI — Kimi Code Master Configuration

> Ported from `/CLAUDE.md`. In the original, this file is loaded automatically every Claude Code
> session via project-root `CLAUDE.md`. For Kimi Code, load it from `.kimi/AGENTS.md`'s pointer
> to `.kimi/context/master-config.md`. See `.kimi/context/reasoning-protocols.md` for the
> `@CLAUDE-REASONING.md` import equivalent.
...
> **Kimi Code notu**: Bu kural agent kimliğinden bağımsızdır — Kimi Code oturumu da aynı
> senkronu yapmalı (aynı script, aynı komut). Bu proje kuralı `.kimi/config.toml` içindeki
> hiçbir ayarla devre dışı bırakılamaz.
...
> **Not (Kimi Code):** `KIMI_API_KEY`/`KIMI_MODEL` yukarıda proje içindeki Moonshot/Kimi LLM API
> entegrasyonuna (bias debate engine) ait — Kimi Code ajanının kendisiyle bir ilgisi yoktur, isim
> çakışmasına dikkat.
