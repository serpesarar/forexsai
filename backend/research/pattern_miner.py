...
# Robust env loading — repo-root usage AND in-backend deploy.
# In production env vars are already in os.environ from Railway.
_HERE = Path(__file__).resolve()
for _candidate in (
    _HERE.parent.parent / ".env",
    _HERE.parent.parent / "backend" / ".env",
    _HERE.parent / ".env",
):
...
def _default_out_dir() -> Path:
    """Where the matcher reads from. backend/data/ when running inside the
    deployed backend container; falls back to this file's own dir for local CLI."""
    backend_data = _HERE.parent.parent / "data"
    if _HERE.parent.name == "research" or backend_data.exists():
...
def run_mining(days: int = 60, min_samples: int = 20,
    ...
    # Render — caller may override out_dir (e.g. backend/data/ in production)
    target_out_dir = out_dir if out_dir is not None else _default_out_dir()
    md = []
    ...
    if write_files:
        ...
        md_path = target_out_dir / "pattern_report.md"
        md_path.write_text("\n".join(md))
        json_path = target_out_dir / "pattern_rules.json"
