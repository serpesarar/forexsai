...
# Robust env loading — works in xauusdegitim/ (repo root) AND backend/research/
...
# When deployed inside backend/research/, backend root is parent.parent
_BACKEND = _HERE.parent.parent
if (_BACKEND / "services").exists() and str(_BACKEND) not in sys.path:
...
def run_chart_mining(symbols: list[str], timeframes: list[str],
                      days: int = 60, write_files: bool = True,
    ...
    target_out = out_dir if out_dir is not None else _default_out_dir()
    ...
    if write_files:
        (target_out / "chart_pattern_report.md").write_text(md)
        (target_out / "chart_pattern_rules.json").write_text(json.dumps(payload, indent=2, default=str))
