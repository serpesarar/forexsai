...
async def get_exchange(exch):
    async with httpx.AsyncClient() as client:
        ...
        if resp.status_code == 200:
            data = resp.json()
            print(f"[{exch}] Total symbols: {len(data)}")
            
            # Print symbols matching "Crude", "Oil", "WTI", "Gold", "XAU"
            for item in data:
                name = item.get('Name', '').upper()
                code = item.get('Code', '').upper()
                if 'OIL' in name or 'WTI' in name or 'CRUDE' in name or 'GOLD' in name or 'XAU' in name or 'BRENT' in name or code in ('CL', 'GC', 'BZ'):
                    print(f"{code}.{exch} - {name} ({item.get('Type')})")
        else:
            print(f"Error for {exch}: {resp.status_code} {resp.text[:100]}")

async def main():
    await get_exchange("COMM")
