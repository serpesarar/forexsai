...
def _desired_payload(entry_price: float, direction: str, symbol: str) -> Dict[str, Any]:
    targets = calculate_target_prices(entry_price, direction, symbol, DESIRED_TIMEFRAME)
    sl_price = calculate_stoploss_price(entry_price, direction, symbol, DESIRED_TIMEFRAME)
...
def plan_xauusd_ml_backfill_update(row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    desired = _desired_payload(entry_price, direction, "XAUUSD")
...
def _fetch_oldest_xauusd_created_at(client) -> Optional[datetime]:
    result = client.table("prediction_logs").select("created_at").eq("symbol", "XAUUSD").order("created_at", desc=False).limit(1).execute()
...
def _fetch_window_rows(client, start: datetime, end: datetime) -> List[Dict[str, Any]]:
    ...
    ).eq("symbol", "XAUUSD").gte("created_at", _utc_iso(start)).lt("created_at", _utc_iso(end)).order("created_at", desc=False).limit(1000).execute()
...
def run_xauusd_ml_history_backfill(
    *,
    ...
    oldest = _fetch_oldest_xauusd_created_at(client)
    if oldest is None:
        return {
            "success": True,
            "dry_run": dry_run,
    ...
    while cursor < end and scanned < max_records:
        ...
        batch_rows = _fetch_window_rows(client, cursor, window_end)
        if not batch_rows:
            cursor = window_end
            continue
        ...
        for row in batch_rows:
            ...
            if normalize_model_type(row) != "ml":
                continue
            ml_considered += 1
            plan = plan_xauusd_ml_backfill_update(row)
            if not plan:
                continue
